...
class _RemindersScreenState extends State<RemindersScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final preferences =
          await AppSettingsService.loadNotificationPreferences();
  ...
  String _reminderStatus(Bill bill) {
    ...
    final normalizedReminder = DateTime(
      reminderDate.year,
      reminderDate.month,
      reminderDate.day,
    );
    ...
    if (normalizedReminder.isAtSameMomentAs(normalizedNow)) {
      return 'Reminder scheduled for today';
    }
    if (normalizedReminder.isBefore(normalizedNow)) {
      return 'Reminder window already started';
    }

    final daysUntilReminder = normalizedReminder
        .difference(normalizedNow)
        .inDays;
                  ...
                  child: ListView(
                    ...
                    children: [
                      ModernTopBar(
                        title: 'Reminders',
                        subtitle:
                            'Keep track of alert windows before each due date.',
                      ...
                      if (activeBills.isEmpty)
                        const EmptyStateCard(
                          ...
                          subtitle:
                              'Add unpaid bills to start seeing reminder schedules here.',
                        )
                      else
                        ...activeBills.map((bill) {
                          final category =
                              BillCategories.findById(bill.categoryId) ??
                              BillCategories.other;
                          final reminderDate = _reminderDate(bill);
                                    ...
                                    Expanded(
                                      child: _ReminderDetailTile(
                                        ...
                                        value: reminderDate == null
                                            ? 'Unavailable'
                                            : _dateFormat.format(reminderDate),
                                      ),
                                    ),
                                    const SizedBox(width: 12),
                                    Expanded(
...
class _ReminderDetailTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return SoftInfoTile(label: label, value: value, icon: icon);
  }
}
